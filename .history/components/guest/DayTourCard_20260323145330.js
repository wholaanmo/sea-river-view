...
export default function DayTourCard({ tour }) {
  ...
  const totalActivitiesPrice = tour.activities?.reduce((sum, activity) => sum + activity.price, 0) || 0;
        ...
        <div className="p-6">
          ...
          <div className="flex gap-4 mt-3 text-sm text-textSecondary border-b border-ocean-light/10 pb-3">
            ...
            {tour.activities && tour.activities.length > 0 && (
              <span className="flex items-center gap-1">
                <i className="fas fa-tag"></i> {tour.activities.length} Add-ons
              </span>
            )}
          </div>
          ...
          {tour.activities && tour.activities.length > 0 && (
            <div className="mt-3">
              <div className="flex flex-wrap gap-1">
                {tour.activities.slice(0, 2).map((activity, idx) => (
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-neutral uppercase tracking-wide mb-1">Activity Add-ons</label>
                <div className="space-y-2">
                  {tour.activities && tour.activities.length > 0 ? (
                    ...
                    <p className="text-textSecondary">No add-on activities available</p>
                  )}
                </div>
              </div>
              <div className="col-span-2">
