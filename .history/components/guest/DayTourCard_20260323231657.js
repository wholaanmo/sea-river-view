...
export default function DayTourCard({ tour }) {
  ...
  const getPriceSuffix = () => {
    if (tour.pricingType === 'per_person') {
      return '/person';
    } else if (tour.pricingType === 'promo') {
      return '';
    }
    return '';
  };
        ...
        <div className="p-6">
          ...
          {tour.pricingType === 'per_person' ? (
            <div className="mt-2 space-y-1">
              <p className="text-lg font-bold text-ocean-mid">
                Adult (16+): ₱{tour.adultPrice?.toLocaleString()}<span className="text-sm font-normal text-textSecondary">/person</span>
              </p>
              <p className="text-lg font-bold text-ocean-mid">
                Kid (15-): ₱{tour.kidPrice?.toLocaleString()}<span className="text-sm font-normal text-textSecondary">/person</span>
              </p>
            </div>
          ) : (
            <div className="mt-2">
              <p className="text-2xl font-bold text-ocean-mid">
                ₱{tour.promoPrice?.toLocaleString()}
              </p>
              <p className="text-sm text-textSecondary">Promo Price</p>
            </div>
          )}
          
          {/* Tour Features */}
          <div className="flex gap-4 mt-3 text-sm text-textSecondary border-b border-ocean-light/10 pb-3">
            {tour.maxCapacity && tour.pricingType !== 'per_person' && (
            ...
            {tour.pricingType === 'per_person' && (
              <span className="flex items-center gap-1">
                <i className="fas fa-tag"></i> Per Person Pricing
              </span>
            )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-textPrimary">{tour.type || 'N/A'}</p>
              </div>
              {tour.maxCapacity && tour.pricingType !== 'per_person' && (
                <div>
              ...
              {tour.pricingType === 'per_person' ? (
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-ocean-mid">₱{tour.promoPrice?.toLocaleString()}</p>
