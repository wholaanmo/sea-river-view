...
export default function CartButton() {
  // Try to get cart context, but don't throw error if not available
  let cart;
  try {
    cart = useCart();
  } catch (e) {
    // If cart context is not available, return a dummy button
    return (
      <button
        onClick={() => {}}
  ...
  const { getCartItemCount, setCartOpen } = cart;
