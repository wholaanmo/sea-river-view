...
export default function DayTourCard({ tour }) {
  ...
  // Get the appropriate price display based on pricing type
  const getPriceDisplay = () => {
    if (tour.price) {
      return `₱${tour.price.toLocaleString()}`;
    }
    return '₱0';
  };
  ...
  const getPriceSuffix = () => {
    if (tour.pricingType === 'per_person') {
      ...
      return '/promo';
    } else if (tour.pricingType === 'group_based') {
      return '/group';
    }
    return '';
  };
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-ocean-mid">
                {getPriceDisplay()}
                <span className="text-sm font-normal text-textSecondary">{getPriceSuffix()}</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-ocean-mid">{getPriceDisplay()}</p>
