...
const handleTourImageRemove = async (imageUrl) => {
  const tourName = tourFormData.name || tourFormData.type || 'Untitled';
  ...
  setPendingImageRemovals(prev => {
    const updated = {
      ...prev,
      tour: [...prev.tour, { url: imageUrl, name: tourName }]
    };
    ...
    const timeout = setTimeout(async () => {
      ...
      setPendingImageRemovals(async (currentPending) => {
        const tourRemovals = currentPending.tour;
        
        if (tourRemovals.length > 0) {
          // Group by tour name
          const groupedByTour = tourRemovals.reduce((acc, removal) => {
            if (!acc[removal.name]) {
              acc[removal.name] = [];
            }
...
const handleActivityImageRemove = async (imageUrl) => {
  ...
  // Set new timeout to batch removals
  const timeout = setTimeout(async () => {
    const pendingActivityRemovals = pendingImageRemovals.activity;
    if (pendingActivityRemovals.length > 0) {
      // Group by activity name (in case multiple activities are edited)
      const groupedByActivity = pendingActivityRemovals.reduce((acc, removal) => {
        if (!acc[removal.name]) {
          acc[removal.name] = [];
        }
        acc[removal.name].push(removal);
      ...
      setPendingImageRemovals(prev => ({ ...prev, activity: [] }));
    }
    
    setRemovalTimeout(null);
  }, 500); // Wait 500ms for more removals before logging
