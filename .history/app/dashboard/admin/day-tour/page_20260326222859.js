...
const handleTourImageRemove = (imageUrl) => {
  ...
  // Add to pending removals grouped by tour
  if (!pendingTourRemovalsRef.current[tourName]) {
    pendingTourRemovalsRef.current[tourName] = [];
  }
  pendingTourRemovalsRef.current[tourName].push(imageUrl);

  // Clear existing timer
  if (tourRemovalTimerRef.current) clearTimeout(tourRemovalTimerRef.current);

  // Set new timer to batch removals
  tourRemovalTimerRef.current = setTimeout(async () => {
    for (const [name, images] of Object.entries(pendingTourRemovalsRef.current)) {
      const removalCount = images.length;
      if (removalCount > 0) {
        await logAdminAction({
          action: 'Updated Day Tour',
          module: 'Day Tour',
          details: `Updated Day Tour '${name}': removed ${removalCount} image${removalCount > 1 ? 's' : ''}.`
        });
      }
    }
