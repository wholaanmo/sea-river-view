...
export default function AdminDayTour() {
  ...
  const handleUpdateTour = async (e) => {
    ...
    try {
      ...
      await logAdminAction({
        ...
        module: 'Day Tour',
        details: `Updated day tour "${selectedTour.type}": ${changes.join(', ')}`
      });
  ...
  const handleActivityInputChange = (e) => {
    ...
    if (name === 'pricePerHour' || name === 'availableInventory') {
      const numValue = parseFloat(value);
      if (value !== '' && (isNaN(numValue) || numValue < 0)) {
        setActivityFormErrors(prev => ({
          ...prev,
          [name]: `${name === 'pricePerHour' ? 'Price' : 'Inventory'} cannot be negative`
        }));
  ...
  const isActivityFormIncomplete = () => {
    return !activityFormData.name.trim() || !activityFormData.pricePerHour || 
           !activityFormData.availableInventory || !activityFormData.description.trim();
  };
  ...
  const handleAddActivity = async (e) => {
    ...
    try {
      ...
      await logAdminAction({
        ...
        module: 'Day Tour',
        details: `Added new activity: ${activityFormData.name} (Price: ₱${parseFloat(activityFormData.pricePerHour).toLocaleString()}/hour, Available: ${parseInt(activityFormData.availableInventory)} units)`
      });
  ...
  const handleUpdateActivity = async (e) => {
    ...
    try {
      ...
      const previousData = {
        ...
        availableInventory: selectedActivity.availableInventory
      };
      ...
      if (previousData.availableInventory !== newData.availableInventory) changes.push(`inventory from ${previousData.availableInventory} to ${newData.availableInventory} units`);
      
      // Log the action
      await logAdminAction({
        action: 'Updated Activity',
        module: 'Day Tour',
        details: `Updated activity "${selectedActivity.name}": ${changes.join(', ')}`
      });
  ...
  const handleArchiveActivity = async (activity) => {
    try {
      ...
      await logAdminAction({
        ...
        module: 'Day Tour',
        details: `Archived activity: ${activity.name} (Price: ₱${activity.pricePerHour.toLocaleString()}/hour, Available: ${activity.availableInventory} units)`
      });
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    const matchesStatus = filterStatus === 'all' || 
      (filterStatus === 'available' && activity.availableInventory > 0) ||
      (filterStatus === 'unavailable' && activity.availableInventory === 0);
  ...
  const totalSlots = dayTours.reduce((sum, tour) => {
    if (tour.maxCapacity) {
      return sum + tour.maxCapacity;
    }
    return sum;
  }, 0);
  
  // For available slots: sum of maxCapacity for tours that are available
  const availableSlots = dayTours.reduce((sum, tour) => {
    if (tour.availability === 'available' && tour.maxCapacity) {
      return sum + tour.maxCapacity;
    }
    return sum;
  }, 0);
  
  // For fully booked: tours that are unavailable (could be treated as fully booked)
  const fullyBooked = dayTours.reduce((sum, tour) => {
    if (tour.availability === 'unavailable' && tour.maxCapacity) {
      return sum + tour.maxCapacity;
    }
    return sum;
  }, 0);
  ...
  const availableActivities = activities.filter(activity => activity.availableInventory > 0).length;
  const unavailableActivities = activities.filter(activity => activity.availableInventory === 0).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-md border border-ocean-light/10 p-5 hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-center justify-between h-full">
              <div>
                <h3 className="text-sm font-semibold text-textSecondary uppercase tracking-wide mb-2">Total Slots</h3>
                <div className="text-3xl font-bold text-textPrimary">{totalSlots}</div>
              </div>
              <i className="fas fa-calendar-alt text-ocean-light text-3xl"></i>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-md border border-ocean-light/10 p-5 hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-center justify-between h-full">
              <div>
                <h3 className="text-sm font-semibold text-textSecondary uppercase tracking-wide mb-2">Available Slots</h3>
                <div className="text-3xl font-bold text-textPrimary">{availableSlots}</div>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-md border border-ocean-light/10 p-5 hover:shadow-lg transition-shadow duration-300">
            <div className="flex items-center justify-between h-full">
              <div>
                <h3 className="text-sm font-semibold text-textSecondary uppercase tracking-wide mb-2">Fully Booked</h3>
                <div className="text-3xl font-bold text-textPrimary">{fullyBooked}</div>
              </div>
