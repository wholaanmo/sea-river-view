...
export default function AdminDayTour() {
  ...
  const handleTourImageUpload = async (e) => {
    ...
    try {
      ...
      // Log each image addition
      for (const imageUrl of uploadedUrls) {
        await logAdminAction({
          action: 'Added Image',
          module: 'Day Tour',
          details: `Added an image in Day Tour "${tourFormData.type || 'Untitled'}"`
        });
      }
  ...
  const handleTourImageRemove = async (imageUrl) => {
    ...
    await logAdminAction({
      ...
      module: 'Day Tour',
      details: `Removed an image in Day Tour "${tourFormData.type || 'Untitled'}"`
    });
  };
  ...
  const handleUpdateTour = async (e) => {
    ...
    try {
      ...
      if (previousData.description !== newData.description) changes.push(`description was updated`);
      ...
      // Log the action with more descriptive details
      await logAdminAction({
    ...
    try {
      ...
      for (const imageUrl of uploadedUrls) {
        await logAdminAction({
          action: 'Added Image',
          module: 'Activity',
          details: `Added an image in Activity "${activityFormData.name || 'Untitled'}"`
        });
      }
  ...
  const handleActivityImageRemove = async (imageUrl) => {
    ...
    await logAdminAction({
      action: 'Removed Image',
      module: 'Activity',
      details: `Removed an image in Activity "${activityFormData.name || 'Untitled'}"`
    });
  };
  ...
  const handleUpdateActivity = async (e) => {
    ...
    try {
      ...
      // Build details of what changed with descriptive wording
      const changes = [];
      if (previousData.name !== newData.name) changes.push(`name from "${previousData.name}" to "${newData.name}"`);
      ...
      let actionDetails = `Updated activity "${selectedActivity.name}"`;
      if (changes.length > 0) {
        if (changes.some(c => c.includes('description'))) {
          actionDetails = `Updated the description of "${selectedActivity.name}"`;
          if (changes.length > 1) {
            const otherChanges = changes.filter(c => !c.includes('description')).join(', ');
            actionDetails += ` and also ${otherChanges}`;
          }
        } else {
          actionDetails += `: ${changes.join(', ')}`;
        }
      }
      
      // Log the action with descriptive wording
      await logAdminAction({
