...
export default function AdminDayTour() {
  ...
  const [confirmDeleteModal, setConfirmDeleteModal] = useState({ show: false, activity: null });
  ...
  const isTourFormIncomplete = () => {
    return !tourFormData.name.trim() || !tourFormData.basePrice || !tourFormData.description.trim();
  };
  ...
  const handleActivityImageRemove = () => {
    setActivityFormData(prev => ({
      ...prev,
      image: ''
    }));
  ...
  const validateActivityForm = () => {
    ...
    if (!activityFormData.image) errors.image = 'Activity image is required';
    
    return errors;
  };
  
  const isActivityFormIncomplete = () => {
    return !activityFormData.name.trim() || !activityFormData.pricePerHour || !activityFormData.description.trim() || !activityFormData.image;
  };
  ...
  const handleDeleteActivity = async () => {
    if (!confirmDeleteModal.activity) return;
    
    setActionLoading(true);
    try {
      await deleteDoc(doc(db, 'activities', confirmDeleteModal.activity.id));
      showNotification('Activity deleted successfully!');
      setConfirmDeleteModal({ show: false, activity: null });
    } catch (error) {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-ocean-ice to-blue-white min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activities.length === 0 ? (
            <div className="col-span-full text-center py-16 bg-white rounded-2xl shadow-md">
              ...
              <p className="text-lg text-neutral">No activities found</p>
              <p className="text-sm text-neutral">Click "Add New Activity" to get started</p>
            </div>
          ) : (
            activities.map((activity) => (
              <div key={activity.id} className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 group">
                {/* Activity Image */}
                <div className="relative h-48 bg-gradient-to-br from-ocean-pale to-ocean-ice overflow-hidden">
                  {activity.image ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <i className="fas fa-bicycle text-6xl text-ocean-light/30"></i>
                    </div>
                  )}
                </div>
                ...
                {/* Activity Details */}
                <div className="p-5">
                  <h3 className="text-xl font-bold text-textPrimary font-playfair mb-2">{activity.name}</h3>
                  <p className="text-2xl font-bold text-ocean-mid mb-2">
                    ₱{activity.pricePerHour.toLocaleString()}
                    <span className="text-sm font-normal text-textSecondary">/hour</span>
                  </p>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-4 py-2 border border-ocean-light/30 text-ocean-mid rounded-xl font-medium hover:bg-ocean-mid hover:text-white hover:border-ocean-mid transition-all duration-300"
                    >
                      <i className="fas fa-edit mr-2"></i> Edit
                    </button>
                    <button
                      onClick={() => setConfirmDeleteModal({ show: true, activity })}
                      className="flex-1 px-4 py-2 border border-red-200 text-red-600 rounded-xl font-medium hover:bg-red-600 hover:text-white hover:border-red-600 transition-all duration-300"
                    >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => {
          ...
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={activityModalType === 'add' ? handleAddActivity : handleUpdateActivity}>
              ...
              {/* Image Upload */}
              <div className="mb-5">
                <label className="block mb-1.5 text-sm font-medium text-textPrimary">Activity Image *</label>
                <div className="border-2 border-dashed border-ocean-light/20 rounded-xl p-4 text-center hover:border-ocean-light transition-colors">
                  <input
                    type="file"
                    accept="image/*"
                ...
                {activityFormData.image && (
                  <div className="mt-3 relative group aspect-video rounded-lg overflow-hidden border border-ocean-light/20">
                    <Image
                      src={activityFormData.image}
                    ...
                    <button
                      type="button"
                      onClick={handleActivityImageRemove}
                ...
                {activityFormErrors.image && <p className="text-red-500 text-xs mt-1">{activityFormErrors.image}</p>}
              </div>
      ...
      {confirmDeleteModal.show && confirmDeleteModal.activity && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-scaleIn">
            <div className="text-center mb-5">
              ...
              <h3 className="text-lg font-bold text-textPrimary mb-2">Delete Activity</h3>
              <p className="text-textSecondary text-sm">
                Are you sure you want to delete "{confirmDeleteModal.activity.name}"? 
                This action cannot be undone.
              </p>
            </div>
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => setConfirmDeleteModal({ show: false, activity: null })}
                className="px-5 py-2 border border-ocean-light/20 rounded-xl text-textSecondary text-sm font-medium hover:bg-ocean-ice transition-all duration-300"
              >
                Cancel
              </button>
