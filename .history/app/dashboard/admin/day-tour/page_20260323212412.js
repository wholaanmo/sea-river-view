...
export default function AdminDayTour() {
  ...
  const isTourFormIncomplete = () => {
    return !tourFormData.name.trim() || !tourFormData.type.trim() || !tourFormData.price || !tourFormData.description.trim();
  };
                      ...
                      filteredTours.map((tour) => (
                        <tr key={tour.id} className="border-b border-ocean-light/10 hover:bg-ocean-ice/30 transition-colors">
                          <td className="px-4 py-3">
                            <div className="font-medium text-textPrimary">{tour.type || 'N/A'}</div>
                            <div className="text-xs text-textSecondary mt-1">{tour.name}</div>
                          </td>
