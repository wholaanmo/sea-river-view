...
export default function AdminDayTour() {
  ...
  const isTourFormIncomplete = () => {
    return !tourFormData.type.trim() || !tourFormData.adultPrice || !tourFormData.kidPrice || !tourFormData.description.trim();
  };
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {filteredTours.length === 0 ? (
                      <tr>
                        <td colSpan="8" className="px-4 py-12 text-center text-neutral">
                      ...
                      filteredTours.map((tour) => (
                        <tr key={tour.id} className="border-b border-ocean-light/10 hover:bg-ocean-ice/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className="font-semibold text-ocean-mid">₱{tour.adultPrice?.toLocaleString()}</span>
                            <span className="text-xs text-neutral ml-1">(16+)</span>
                          </td>
                          <td className="px-4 py-3">
                            <span className="font-semibold text-ocean-mid">₱{tour.kidPrice?.toLocaleString()}</span>
                            <span className="text-xs text-neutral ml-1">(15-)</span>
                          </td>
                          <td className="px-4 py-3">
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-ocean-mid">₱{selectedTour.adultPrice?.toLocaleString()}</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-neutral uppercase tracking-wide mb-1">Kid Price (15-)</label>
                <p className="text-2xl font-bold text-ocean-mid">₱{selectedTour.kidPrice?.toLocaleString()}</p>
              </div>
              <div>
