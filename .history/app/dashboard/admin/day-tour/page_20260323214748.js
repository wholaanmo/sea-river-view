...
export default function AdminDayTour() {
  ...
  const [tourFormData, setTourFormData] = useState({
    ...
    description: '',
    price: '',
    pricingType: 'per_person', // 'per_person', 'promo', 'group_based'
    maxCapacity: '',
    type: '',
  ...
  const pricingTypes = [
    ...
    { value: 'group_based', label: 'Group-based Price' }
  ];
  ...
  const handleTourInputChange = (e) => {
    ...
    if (name === 'price' || name === 'maxCapacity') {
      const numValue = parseFloat(value);
      if (value !== '' && (isNaN(numValue) || numValue < 0)) {
        setTourFormErrors(prev => ({
          ...prev,
          [name]: `${name === 'price' ? 'Price' : 'Maximum capacity'} cannot be negative`
        }));
  ...
  // Handle pricing type change - disable max capacity if per_person is selected
  const handlePricingTypeChange = (e) => {
    const value = e.target.value;
    setTourFormData(prev => ({
      ...prev,
      pricingType: value,
      // If pricing type is per_person, clear max capacity
  ...
  const validateTourForm = () => {
    ...
    // Day tour name is now optional
    if (!tourFormData.type.trim()) errors.type = 'Tour type is required';
    if (!tourFormData.price) errors.price = 'Price is required';
    else if (isNaN(tourFormData.price) || parseFloat(tourFormData.price) <= 0) {
      errors.price = 'Price must be a positive number';
    }
    // Only validate max capacity if pricing type is not per_person
  ...
  const isTourFormIncomplete = () => {
    // Day tour name is now optional, so don't check for it
    return !tourFormData.type.trim() || !tourFormData.price || !tourFormData.description.trim();
  };
  ...
  const handleAddTour = async (e) => {
    ...
    try {
      const tourData = {
        name: tourFormData.name || 'Untitled Tour', // Default name if empty
  ...
  // Activity Handlers with negative number validation
  ...
  const filteredActivities = activities.filter(activity => {
    ...
    // Add status filter for activities
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-ocean-ice to-blue-white min-h-screen">
      ...
      {activeTab === 'tours' && (
        <>
          {/* Filters and Search - Wider Search Bar */}
                    ...
                    {filteredTours.length === 0 ? (
                      ...
                      filteredTours.map((tour) => (
                        <tr key={tour.id} className="border-b border-ocean-light/10 hover:bg-ocean-ice/30 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div className="font-semibold text-ocean-mid">₱{tour.price?.toLocaleString()}</div>
                            <div className="text-xs text-neutral mt-0.5">{getPricingLabel(tour.pricingType)}</div>
                          </td>
                          <td className="px-4 py-3">
      ...
      {/* Activities Tab Content with Search and Status Filter */}
      {activeTab === 'activities' && (
        <>
          {/* Activity Search Bar - Wider */}
          <div className="flex gap-4 mb-6 flex-wrap">
            <div className="flex-1 min-w-[350px]">
              <div className="relative">
                <i className="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-neutral text-sm"></i>
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-2xl font-bold text-textPrimary font-playfair">
                {selectedTour.name || 'Untitled Tour'} - Day Tour Details
              </h2>
              <button
                onClick={() => {
                  setShowTourDetailsModal(false);
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-ocean-mid">₱{selectedTour.price?.toLocaleString()}</p>
                <span className="text-xs text-neutral">{getPricingLabel(selectedTour.pricingType)}</span>
              </div>
              <div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={tourModalType === 'add' ? handleAddTour : handleUpdateTour}>
              ...
              <div className="mb-4">
                ...
                />
                <p className="text-xs text-neutral mt-1">If left empty, will default to "Untitled Tour"</p>
              </div>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 border ${tourFormErrors.price ? 'border-red-500' : 'border-ocean-light/20'} rounded-xl text-sm focus:outline-none focus:border-ocean-light`}
                  step="0.01"
                  min="0"
                />
                {tourFormErrors.price && <p className="text-red-500 text-xs mt-1">{tourFormErrors.price}</p>}
              </div>
              
              {/* Max Capacity - Disabled when Price per person is selected */}
              <div className="mb-4">
                <label className="block mb-1.5 text-sm font-medium text-textPrimary">
                  Maximum Capacity
