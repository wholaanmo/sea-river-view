...
const handleTourImageRemove = async (imageUrl) => {
  ...
  const currentImages = [...tourFormData.images];
  const tourName = tourFormData.name || tourFormData.type || 'Untitled';
  
  // We need to know if this is a single removal or part of a batch
  // Since the UI handles one click at a time, this will be a single removal
  // But we'll format the log to show "removed 1 image"
  setTourFormData(prev => ({
    ...prev,
  ...
  await logAdminAction({
    action: 'Updated Day Tour',
    module: 'Day Tour',
    details: `Updated Day Tour '${tourName}': removed 1 image.`
  });
};
...
const handleUpdateTour = async (e) => {
    ...
    try {
      ...
      for (const imageUrl of uploadedUrls) {
        ...
        await logAdminAction({
          ...
          module: 'Day Tour',
          details: `Updated Activity '${activityName}': added an image.`
        });
      }
  ...
  const handleActivityImageRemove = async (imageUrl) => {
    ...
    await logAdminAction({
      action: 'Updated Activity',
      module: 'Day Tour',
      details: `Updated Activity '${activityName}': removed an image.`
    });
  };
