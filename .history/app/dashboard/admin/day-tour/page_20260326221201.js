...
export default function AdminDayTour() {
  ...
  const [pendingImageRemovals, setPendingImageRemovals] = useState({ tour: [], activity: [] });
...
const handleTourImageUpload = async (e) => {
  ...
  try {
    ...
    const tourName = tourFormData.name || tourFormData.type || 'Untitled';
    const imageCount = uploadedUrls.length;
    await logAdminAction({
      action: 'Updated Day Tour',
      module: 'Day Tour',
      details: `Updated Day Tour '${tourName}': added ${imageCount} image${imageCount > 1 ? 's' : ''}.`
    });
...
// Track pending removals with a ref to avoid state issues
...
const handleTourImageRemove = (imageUrl) => {
  ...
  // Add to pending removals with the tour name
  pendingTourRemovalsRef.current.push({ url: imageUrl, name: tourName });
  ...
  removalTimerRef.current = setTimeout(async () => {
    const pendingRemovals = pendingTourRemovalsRef.current;
    
    if (pendingRemovals.length > 0) {
      // Group by tour name
      const groupedByTour = pendingRemovals.reduce((acc, removal) => {
        if (!acc[removal.name]) {
          acc[removal.name] = [];
        }
        acc[removal.name].push(removal);
        return acc;
      }, {});
      
      // Log each group
      for (const [tourName, removals] of Object.entries(groupedByTour)) {
        const removalCount = removals.length;
        await logAdminAction({
          action: 'Updated Day Tour',
          module: 'Day Tour',
          details: `Updated Day Tour '${tourName}': removed ${removalCount} image${removalCount > 1 ? 's' : ''}.`
        });
      }
      
      // Clear pending removals
      pendingTourRemovalsRef.current = [];
    }
...
const handleActivityImageRemove = (imageUrl) => {
  ...
  // Add to pending removals with the activity name
  pendingActivityRemovalsRef.current.push({ url: imageUrl, name: activityName });
  ...
  // Clear existing timer
  if (activityRemovalTimerRef.current) {
    clearTimeout(activityRemovalTimerRef.current);
  }
  
  // Set new timer to batch removals
  activityRemovalTimerRef.current = setTimeout(async () => {
    const pendingRemovals = pendingActivityRemovalsRef.current;
    
    if (pendingRemovals.length > 0) {
      // Group by activity name
      const groupedByActivity = pendingRemovals.reduce((acc, removal) => {
        if (!acc[removal.name]) {
          acc[removal.name] = [];
        }
        acc[removal.name].push(removal);
      ...
      // Log each group
      for (const [activityName, removals] of Object.entries(groupedByActivity)) {
        const removalCount = removals.length;
        await logAdminAction({
          action: 'Updated Activity',
          module: 'Day Tour',
          details: `Updated Activity '${activityName}': removed ${removalCount} image${removalCount > 1 ? 's' : ''}.`
        });
      }
...
const handleActivityImageRemove = async (imageUrl) => {
  const activityName = activityFormData.name || 'Untitled';
  ...
  setPendingImageRemovals(prev => {
    const updated = {
      ...prev,
      activity: [...prev.activity, { url: imageUrl, name: activityName }]
    };
    ...
    // Set new timeout to batch removals
    const timeout = setTimeout(async () => {
      // Get the current pending removals at execution time
      setPendingImageRemovals(async (currentPending) => {
        const activityRemovals = currentPending.activity;
        
        if (activityRemovals.length > 0) {
          // Group by activity name
          const groupedByActivity = activityRemovals.reduce((acc, removal) => {
            if (!acc[removal.name]) {
              acc[removal.name] = [];
            }
            acc[removal.name].push(removal);
        ...
        return { ...currentPending, activity: [] };
      });
    }, 500);
