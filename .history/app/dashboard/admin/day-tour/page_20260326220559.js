...
const handleTourImageRemove = async (imageUrl) => {
  ...
  const timeout = setTimeout(async () => {
    const pendingTourRemovals = pendingImageRemovals.tour;
    if (pendingTourRemovals.length > 0) {
      // Group by tour name (in case multiple tours are edited)
      const groupedByTour = pendingTourRemovals.reduce((acc, removal) => {
        if (!acc[removal.name]) {
          acc[removal.name] = [];
        }
      ...
      setPendingImageRemovals(prev => ({ ...prev, tour: [] }));
    }
