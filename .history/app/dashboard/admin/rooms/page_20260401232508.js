...
export default function AdminRooms() {
  ...
  const handleInputChange = (e) => {
    ...
    // For numeric fields, prevent negative numbers
  ...
  // Update availability status automatically based on room data
  useEffect(() => {
    if (modalType === 'edit' && selectedRoom) {
      const totalRooms = parseInt(formData.totalRooms);
      const maintenanceRooms = parseInt(formData.maintenanceRooms) || 0;
      
      if (!isNaN(totalRooms) && totalRooms > 0) {
        if (totalRooms > maintenanceRooms) {
          setFormData(prev => ({ ...prev, availability: 'available' }));
        } else if (totalRooms === maintenanceRooms) {
          setFormData(prev => ({ ...prev, availability: 'maintenance' }));
        }
      }
    }
  }, [formData.totalRooms, formData.maintenanceRooms, modalType, selectedRoom]);
