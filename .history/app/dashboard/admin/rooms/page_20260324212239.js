  ...
  const handleAddRoom = async (e) => {
    ...
    try {
      ...
      await logAuditAction({
        action: 'Created room',
        module: 'Rooms',
        details: `Added new room type: ${roomData.type} (Capacity: ${roomData.capacity} guests, Price: ₱${roomData.price.toLocaleString()}, Total Rooms: ${roomData.totalRooms})`,
        itemId: docRef.id,
        itemName: roomData.type
      });
  ...
  const handleUpdateRoom = async (e) => {
    ...
    try {
      ...
      const previousData = {
        ...
        inclusions: selectedRoom.inclusions?.length || 0,
  ...
  const handleArchiveRoom = async (room) => {
    try {
      ...
      await logAuditAction({
        ...
        module: 'Rooms',
        details: `Archived room: ${room.type} (Capacity: ${room.capacity} guests, Price: ₱${room.price.toLocaleString()})`,
        itemId: room.id,
        itemName: room.type
      });
