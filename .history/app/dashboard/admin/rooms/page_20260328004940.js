...
const [formData, setFormData] = useState({
  type: '',
  totalRooms: '',
  capacityMin: '',  // Changed from 'capacity'
  capacityMax: '',  // New field for max capacity
  inclusions: [],
  price: '',
...
const handleImageUpload = async (e) => {
  ...
  if (!formData.capacityMin) errors.capacityMin = 'Minimum capacity is required';
  else if (isNaN(formData.capacityMin) || parseInt(formData.capacityMin) <= 0) {
    errors.capacityMin = 'Minimum capacity must be a positive number';
  }
  if (!formData.capacityMax) errors.capacityMax = 'Maximum capacity is required';
  else if (isNaN(formData.capacityMax) || parseInt(formData.capacityMax) <= 0) {
    errors.capacityMax = 'Maximum capacity must be a positive number';
  }
  if (formData.capacityMin && formData.capacityMax && 
      parseInt(formData.capacityMin) > parseInt(formData.capacityMax)) {
...
const handleAddRoom = async (e) => {
  ...
  try {
    ...
    await logAdminAction({
      ...
      details: `Added new room: ${roomData.type} (Capacity: ${roomData.capacityMin}–${roomData.capacityMax} guests, Price: ₱${roomData.price.toLocaleString()}, Total Rooms: ${roomData.totalRooms}, Status: ${roomData.availability})`
    });
...
const detectChanges = () => {
  ...
  return hasTypeChanged || hasTotalRoomsChanged || hasCapacityMinChanged || 
         hasCapacityMaxChanged || hasPriceChanged || hasAvailabilityChanged || 
         hasDescriptionChanged || hasInclusionsChanged || hasImagesChanged;
};
  ...
  const maintenanceRooms = rooms.reduce((sum, room) => {
