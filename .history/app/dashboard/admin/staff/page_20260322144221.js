...
export default function StaffManagement() {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      // Get user by email from Firebase Auth (this requires admin SDK, but we'll use a workaround)
  ...
  const getStatusDisplay = (status, emailVerified) => {
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-ocean-ice to-blue-white min-h-screen">
      {/* Header */}
            ...
            <tbody>
              {filteredStaff.length === 0 ? (
                ...
                filteredStaff.map((member) => {
                  const statusDisplay = getStatusDisplay(member.status, member.emailVerified);
                  return (
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-xl text-white text-sm font-medium transition-all duration-300 ${
                      actionLoading || isFormIncomplete()
                        ? 'bg-neutral cursor-not-allowed opacity-60'
