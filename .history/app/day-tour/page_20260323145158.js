...
export default function AdminDayTour() {
  ...
  const handleActivityAdd = () => {
    if (activityInput.name.trim() && activityInput.price) {
      setFormData(prev => ({
        ...prev,
        activities: [...prev.activities, { ...activityInput, id: Date.now() }]
      }));
      setActivityInput({ name: '', price: '', description: '' });
    }
  };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-textSecondary">
            Manage your day tour packages, pricing, and activities
          </p>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-ocean-pale/50 border-b border-ocean-light/20">
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-textPrimary">Activities</th>
          ...
          <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-semibold text-neutral uppercase tracking-wide mb-1">Activities</label>
                <div className="space-y-2">
                  {selectedTour.activities && selectedTour.activities.length > 0 ? (
                    selectedTour.activities.map((activity, idx) => (
                      <div key={idx} className="bg-ocean-ice/50 rounded-lg p-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-semibold text-textPrimary">{activity.name}</h4>
                            {activity.description && (
                              <p className="text-sm text-textSecondary mt-1">{activity.description}</p>
                            )}
                          </div>
                          <p className="font-bold text-ocean-mid">₱{activity.price.toLocaleString()}</p>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-textSecondary">No activities available</p>
                  )}
                </div>
              </div>
              <div className="col-span-2">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => {
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-auto p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={modalType === 'add' ? handleAddTour : handleUpdateTour}>
              ...
              {/* Activities Add-ons */}
              <div className="mb-4">
                <label className="block mb-1.5 text-sm font-medium text-textPrimary">Activity Add-ons</label>
                <div className="bg-ocean-ice/30 rounded-xl p-4 mb-3">
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <input
                      type="text"
                      placeholder="Activity name (e.g., ATV)"
                      value={activityInput.name}
                      onChange={(e) => setActivityInput({ ...activityInput, name: e.target.value })}
                      className="px-3 py-2 border border-ocean-light/20 rounded-lg text-sm focus:outline-none focus:border-ocean-light"
                    />
                    <input
                      type="number"
                      placeholder="Price (₱)"
                      value={activityInput.price}
                      onChange={(e) => setActivityInput({ ...activityInput, price: e.target.value })}
                      className="px-3 py-2 border border-ocean-light/20 rounded-lg text-sm focus:outline-none focus:border-ocean-light"
                    />
                  </div>
                  <textarea
                    ...
                    rows="2"
                    className="w-full px-3 py-2 border border-ocean-light/20 rounded-lg text-sm focus:outline-none focus:border-ocean-light mb-3"
                  />
                  <button
                    type="button"
                    onClick={handleActivityAdd}
                    className="w-full px-4 py-2 bg-ocean-light/10 text-ocean-light rounded-lg hover:bg-ocean-light hover:text-white transition-all"
                  >
                ...
                {formData.activities.length > 0 && (
                  <div className="space-y-2">
                    {formData.activities.map((activity) => (
                      <div key={activity.id} className="flex justify-between items-center bg-white border border-ocean-light/20 rounded-lg p-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-textPrimary">{activity.name}</span>
                            <span className="text-sm font-semibold text-ocean-mid">₱{activity.price.toLocaleString()}</span>
                          </div>
                          {activity.description && (
                            <p className="text-xs text-textSecondary mt-1">{activity.description}</p>
                          )}
                        </div>
                        <button
                          type="button"
