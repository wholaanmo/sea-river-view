...
export default function RoomCalendar() {
  ...
  const isDateAvailable = (date) => {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      const isTimeSlotBooked = bookedTimes.includes(timeKey);
      
      // If this time slot is not booked, it's available
      if (!isTimeSlotBooked) {
        availableTimeSlots++;
      }
    }
...
<div className="flex gap-6">
  ...
  <div className="w-4/5">
    <div className="text-sm font-semibold text-textSecondary mb-2">Select Check-in Date</div>

    {/* Calendar Grid */}
    <div className="grid grid-cols-7 gap-3 mb-4">
      {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(day => (
        <div key={day} className="text-center font-semibold text-textSecondary py-3">{day}</div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-7 gap-3">
      {days.map((day, index) => {
        ...
        let hoverClass = 'hover:bg-ocean-ice';
        ...
        return (
          <button
            key={index}
            onClick={() => !isPast && !fullyBooked && handleDateSelect(day)}
  ...
  <div className="w-1/5">
    {selectedDate ? (
      <div className="bg-ocean-ice rounded-xl p-4 sticky top-4">
        <h3 className="text-sm font-semibold text-textPrimary mb-3">Select Check-in Time</h3>
        <div className="space-y-2 max-h-[400px] overflow-y-auto">
          {timeSlots.map(slot => {
            ...
            return (
              ...
              >
                {slot.display}
                {isOccupied && <span className="block text-xs text-red-500">Occupied</span>}
              </button>
            );
          })}
        </div>
