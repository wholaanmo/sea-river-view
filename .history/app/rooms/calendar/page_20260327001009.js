...
export default function RoomCalendar() {
  ...
  const [bookedDates, setBookedDates] = useState({}); // Store occupancy per date and time slots
  ...
  const fetchBookings = async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Track which time slots are occupied for each date
        ...
        while (current < checkOut) {
          ...
          // Add the number of rooms booked
          const roomsBooked = booking.numberOfRooms || 1;
          booked[dateKey].count += roomsBooked;
          
          // Store the check-in time slot for this date
          const timeKey = `${checkIn.getHours()}:0`;
          booked[dateKey].timeSlots.add(timeKey);
      ...
      const processedBooked = {};
      Object.keys(booked).forEach(dateKey => {
        processedBooked[dateKey] = {
          count: booked[dateKey].count,
          timeSlots: Array.from(booked[dateKey].timeSlots)
        };
      });
      
      setBookedDates(processedBooked);
      setLoading(false);
    } catch (error) {
  ...
  // FIX: A date should be marked as red (fully booked) only if all time slots for that day are no longer available
  const isDateAvailable = (date) => {
    ...
    const occupiedTimeSlots = bookedDates[dateKey]?.timeSlots || [];
    const totalRoomsAvailable = roomDetails?.totalRooms || 1;
    
    // Count how many time slots are still available
    let availableTimeSlots = 0;
    for (let hour = 0; hour < 24; hour++) {
      const timeKey = `${hour}:0`;
      const isTimeSlotOccupied = occupiedTimeSlots.includes(timeKey);
      
      // If this time slot is not occupied, it's available
      if (!isTimeSlotOccupied) {
        availableTimeSlots++;
      }
    }
    
    // Date is available if there's at least one available time slot
  ...
  const isTimeSlotAvailable = (date, hour) => {
    ...
    const timeKey = `${hour}:0`;
    return !occupiedTimeSlots.includes(timeKey);
  };

  // Check if a date is fully booked (all time slots are occupied)
  const isDateFullyBooked = (date) => {
    if (!date) return false;
    const dateKey = date.toDateString();
    const occupiedTimeSlots = bookedDates[dateKey]?.timeSlots || [];
    
    // Check if all 24 time slots are occupied
    for (let hour = 0; hour < 24; hour++) {
      const timeKey = `${hour}:0`;
      if (!occupiedTimeSlots.includes(timeKey)) {
        return false; // Found an available time slot, so not fully booked
      }
    }
    return true; // All time slots are occupied
  };
        ...
        <div className="max-w-6xl w-full px-4">
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div className="p-8">
              ...
              {/* Layout: Box 1 (80% width) and Box 2 (20% width) aligned in a single row */}
              <div className="flex gap-6">
                {/* Box 1: Check-in Date Section - 80% width */}
                <div className="w-4/5">
                  {/* "Select a date first" text moved outside */}
                  {!selectedDate && (
                    <div className="mb-4 text-center">
                      <p className="text-textSecondary text-sm">Select a date first to see available time slots</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {days.map((day, index) => {
                      ...
                      if (isPast) {
                        ...
                        // Only mark as red if fully booked (no available time slots)
                        bgColor = 'bg-red-100';
                        ...
                        // Dates with no available slots but not fully booked? This shouldn't happen with current logic
                        bgColor = 'bg-gray-100';
                        textColor = 'text-gray-400';
                        borderClass = 'border border-gray-200';
                        cursorClass = 'cursor-not-allowed';
                      ...
                      return (
                        <button
                          key={index}
                          onClick={() => !isPast && !isFullyBooked && isAvailable && handleDateSelect(day)}
                          disabled={isPast || isFullyBooked || !isAvailable}
                ...
                {/* Box 2: Check-in Time Section - 20% width (Separate box outside Box 1) */}
                <div className="w-1/5">
                  {selectedDate ? (
                    ...
                    <div className="bg-ocean-ice rounded-xl p-4 text-center">
                      ...
                      <p className="text-sm text-textSecondary">Select Check-in Time</p>
                      <p className="text-xs text-textSecondary mt-1">Choose a date first</p>
                    </div>
                  )}
                </div>
              </div>
