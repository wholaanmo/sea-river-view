...
export default function RoomCalendar() {
  ...
  const [bookedSlots, setBookedSlots] = useState({}); // Store booked time slots per date
  ...
  const fetchBookings = async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Get the check-in time slot (hour)
        const checkInHour = checkIn.getHours();
        const timeKey = `${checkInHour}:00`;
        
        // For each night of the stay, mark that time slot as booked
        let current = new Date(checkIn);
        while (current < checkOut) {
          const dateKey = current.toDateString();
          if (!booked[dateKey]) {
            booked[dateKey] = new Set();
          }
          // Mark this specific time slot as booked for this date
          booked[dateKey].add(timeKey);
      ...
      Object.keys(booked).forEach(dateKey => {
        bookedSlotsArray[dateKey] = Array.from(booked[dateKey]);
      });
  ...
  const isDateAvailable = (date) => {
    ...
    // If no time slots are booked, all 24 slots are available
    if (bookedTimes.length === 0) return true;
    
    // Check if there's at least one available time slot (24 total slots)
    for (let hour = 0; hour < 24; hour++) {
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div className="p-8">
              ...
              <div className="flex gap-6">
                {/* Check-in Date Section - 80% width (Box 1) */}
                <div className="w-4/5">
                  {/* Calendar Grid */}
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {days.map((day, index) => {
                      ...
                      if (isPast) {
                        ...
                        // Mark as red only if fully booked (all time slots are occupied)
                        bgColor = 'bg-red-100';
                ...
                {/* Check-in Time Section - 20% width (Box 2) */}
                <div className="w-1/5">
                  {selectedDate ? (
