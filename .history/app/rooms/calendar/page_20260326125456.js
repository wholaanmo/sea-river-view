...
export default function RoomCalendar() {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        while (current < checkOut) {
          ...
          // Store the check-in time for this booking
          const timeKey = `${checkIn.getHours()}:0`;
          if (!booked[dateKey].times.includes(timeKey)) {
  ...
  /**
   * FIX: Check if a date is fully booked (all 24 time slots are occupied)
   * Returns true if there is at least ONE available time slot
   * Returns false if ALL 24 time slots are booked (fully booked)
   */
  const isDateAvailable = (date) => {
    ...
    // If there are no bookings at all for this date, it's fully available
    if (bookedTimes.length === 0) {
      return true;
    }
    
    // Check all 24 hours (0-23) to see if any time slot is available
    for (let hour = 0; hour < 24; hour++) {
      const timeKey = `${hour}:0`;
      // If this time slot is NOT in the bookedTimes array, it's available
      if (!bookedTimes.includes(timeKey)) {
        return true; // Found at least one available time slot
      }
    }
    
    // If we get here, all 24 time slots are booked
    return false;
  };
                ...
                <div className="w-4/5">
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {days.map((day, index) => {
                      ...
                      const bookedTimes = bookedDates[day.toDateString()]?.times || [];
                      ...
                      if (isPast) {
                        ...
                        // Date is fully booked (all 24 time slots are occupied)
                        bgColor = 'bg-red-100';
