...
export default function RoomCalendar() {
  ...
  // Check each time slot to see if any are available
  for (let hour = 0; hour < 24; hour++) {
...
const isDateAvailable = (date) => {
  ...
  // Date is available if booked rooms count is less than total rooms
...
const isTimeSlotAvailable = (date, hour) => {
  ...
  const isSlotOccupied = bookedTimes.includes(timeKey);
  
  // Also need to check if the total rooms for that date are fully booked
  const totalBookedCount = bookedDates[dateKey]?.count || 0;
  const totalRoomsAvailable = roomDetails?.totalRooms || 1;
  
  // If total rooms are fully booked, no time slots are available
  if (totalBookedCount >= totalRoomsAvailable) {
    return false;
  }
  
  // Otherwise, check if this specific time slot is occupied
  ...
  <div className="w-4/5">
    ...
    <div className="grid grid-cols-7 gap-3">
      {days.map((day, index) => {
        ...
        const hasAvailableTimeSlot = checkAvailableTimeSlots(day);
        ...
        if (isPast) {
          ...
          // Date is fully booked if no time slots are available
          bgColor = 'bg-red-100';
        ...
        return (
          <button
            key={index}
            onClick={() => !isPast && hasAvailableTimeSlot && handleDateSelect(day)}
            disabled={isPast || !hasAvailableTimeSlot}
