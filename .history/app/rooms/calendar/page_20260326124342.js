...
export default function RoomCalendar() {
   ...
   * If there is at least one available time slot, the date should remain available (normal color).
   */
  const isDateAvailable = (date) => {
    ...
    // A date is available if there is at least ONE time slot that is NOT booked
    // We check all 24 hours (0-23)
    for (let hour = 0; hour < 24; hour++) {
      const timeKey = `${hour}:0`;
      if (!bookedTimes.includes(timeKey)) {
        // Found an available time slot, date is available
        return true;
      }
    }
    
    // No available time slots found - date is fully booked
    return false;
  };
