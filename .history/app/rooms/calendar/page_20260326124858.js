  ...
  const fetchBookings = async () => {
    try {
      ...
      const q = query(
        bookingsRef, 
        where('roomId', '==', roomId),
        where('status', 'in', ['pending', 'confirmed', 'check-in']) // Include pending to show occupied slots
      );
      const querySnapshot = await getDocs(q);
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Track occupancy per date and time slot
        ...
        while (current < checkOut) {
          ...
          // Add the number of rooms booked (default to 1)
...
const isDateAvailable = (date) => {
  ...
  const bookingsForDate = bookedDates[dateKey];
  const totalRoomsAvailable = roomDetails?.totalRooms || 1;

  // If no bookings at all, fully available
  if (!bookingsForDate) return true;

  // Count bookings per hour
  const timeCount = {};
  bookingsForDate.times.forEach((time) => {
  ...
  for (let hour = 0; hour < 24; hour++) {
    ...
    const bookedCount = timeCount[timeKey] || 0;

    // If at least one slot is still available → date is AVAILABLE
    if (bookedCount < totalRoomsAvailable) {
      return true;
    }
  }

  // If ALL time slots reached max rooms → FULLY BOOKED
  return false;
};
...
const isTimeSlotAvailable = (date, hour) => {
  ...
  const bookedCount = bookingsForDate.times.filter(t => t === timeKey).length;

  // Available only if below max rooms
  return bookedCount < totalRoomsAvailable;
};
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div className="p-8">
              ...
              <div className="flex gap-6">
                {/* Check-in Date Section - 80% width (Container 1) */}
                <div className="w-4/5">
                  {/* Calendar Grid */}
                ...
                {/* Check-in Time Section - 20% width (Container 2) */}
                <div className="w-1/5">
                  {selectedDate ? (
