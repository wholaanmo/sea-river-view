...
const handleSubmitBooking = async () => {
  ...
  try {
    ...
    // We need to count pending as well to prevent overbooking
    const q = query(
      bookingsRef,
      where('roomId', '==', bookingData.roomId),
      where('status', 'in', ['confirmed', 'check-in', 'pending']), // Include pending bookings
...
const checkAvailability = async () => {
  ...
  try {
    ...
    const requestedRooms = 1; // Each booking is for 1 room by default
...
{step === 1 && (
  <div className="bg-white rounded-2xl shadow-lg p-8">
    ...
    {/* Add availability warning at the top if needed */}
