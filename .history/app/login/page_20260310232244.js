...
export default function Login() {
    ...
    // Admin credentials (for special admin access)
    ...
    const loginUser = async (e) => {
        ...
        try {
            // Special admin check (bypass Firebase for admin)
            ...
            // Firebase authentication for all other users
            const userCredential = await signInWithEmailAndPassword(auth, email, password);
            const user = userCredential.user;
            
            // Get user's role from Firebase (you can store this in Firestore)
            // For now, we'll determine role based on email or userType selection
            let role = userType; // Use the selected user type as role
